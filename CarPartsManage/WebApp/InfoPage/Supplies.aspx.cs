...
namespace WebApp.InfoPage
{
    public partial class Supplies : System.Web.UI.Page
    {
        ...
        { 
            //查询数据并绑定显示
            string sql = "SELECT Id,Num,Name,Principal,Address,Ways,Phone FROM Supplies;";
            DataSet ds = DbHelperSQL.Query(sql);
            gvSupplies.DataSource = ds;
            gvSupplies.DataBind();
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow gvr in gvSupplies.Rows)
            {
                CheckBox cbSelection = gvr.FindControl("cbSelection") as CheckBox;
                if (cbSelection.Checked)
                {
                    sbIds.Append(gvSupplies.DataKeys[gvr.RowIndex]["Id"] + ",");
                }
            }
