...
namespace WebApp.InfoPage
{
    public partial class PartsInfo : System.Web.UI.Page
    {
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow gvr in gvParts.Rows)
            {
                CheckBox cbSelection = gvr.FindControl("cbSelection") as CheckBox;
                if (cbSelection.Checked)
                {
                    sbIds.Append(gvParts.DataKeys[gvr.RowIndex]["PartId"] + ",");
                }
            }
            ...
            if (sbIds.Length - 1 > 1)
            {
                sbIds.Insert(0, "DELETE FROM dbo.CarParts WHERE PartId in (");
                sbIds.Insert(sbIds.Length, ")");
            }
            else
            {
                sbIds.Insert(0, "DELETE FROM dbo.CarParts WHERE PartId = ");
            }
            DbHelperSQL.ExecuteSql(sbIds.ToString());
            Response.Write("<script>alert('删除成功');</script>");
            Response.Redirect("PartsInfo.aspx");
        }
    }
}